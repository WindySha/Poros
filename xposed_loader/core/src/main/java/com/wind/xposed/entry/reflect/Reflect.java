...
public class Reflect {
    ...
    public static Object defaultValue(Class<?> paramType) {
        Class<?> type = wrapper(paramType);
        if (type == null) {
            return null;
        } else if (type.isPrimitive()) {
            if (Boolean.class == type) {
    ...
    private Field field0(String name) throws ReflectException {
        ...
        } catch (NoSuchFieldException e) { // 此时尝试非公有字段
            do {
                try {
    ...
    public Reflect callBest(String name, Object... args) throws ReflectException {
        ...
        if (bestMethod != null) {
            ...
            throw new ReflectException("no method found for " + name,
                    new NoSuchMethodException("No best method " + name + " with params " + Arrays.toString(types)
                            + " could be found on type " + type() + "."));
        }
    }
